...
<html>
	...
	<body>
			...
			<div class="layui-form-item">
				<div class="layui-inline">
					...
					<div class="layui-input-inline">
						<input type="text" name="name" placeholder="请输入" value="${(user.name)!}" class="layui-input"/>
					</div>
				</div>
				<div class="layui-inline">
					<label class="layui-form-label">手机</label>
					<div class="layui-input-block">
						<input type="text" name="phone" placeholder="请输入" value="${(user.phone)!}" class="layui-input"/>
					</div>
				</div>
				<div class="layui-inline">
					<label class="layui-form-label">编码</label>
					<div class="layui-input-block">
						<input type="text" name="code" placeholder="请输入" value="${(user.code)!}" class="layui-input"/>
					</div>
				</div>
			</div>
			</form>
			<blockquote class="layui-elem-quote">
				<a href="${webRoot}/sys/user/add" class="layui-btn layui-btn-small" id="add">
		...
		<script>
			...
			function page(curr){
				$.ajax({
					url: "${webRoot}/sys/user/search?pageNo=" + curr,
			 ...
			 $('#search').on('click', function() {
				$.post("${webRoot}/sys/user/list", $("#search_form").serialize(),
			    function(data) {
			    	page(1);
			    });
			 });
